# Changelog

All notable changes to `laneros-browser-extension` will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## 10.1.3 | 2021-03-03

### Changed

- Mark as read/unread an Alert is now operative

## 10.1.2 | 2021-02-22

### Changed

- Background function fix

## 10.1.1 | 2021-02-21

### Changed

- Tailwind version upgraded

## 10.1 | 2021-02-21

### Changed

- New logo
- Feedback is back (again)
- Fixed review time calculation
- HTML, CSS and JS improvements

## 10.0.1 | 2021-02-04

### Changed

- Fixed avatar url (best image quality)
- Fixed background data verification

## 10.0 | 2021-02-04

### Added

- Quick actions

### Changed

- Each tab is self-managed
- Code reorganization
- Some bug fixed
    

## 9.2.1 | 2020-12-18

### Changed

- Change alerts tab behaviour
- Minor HTML fixes

## 9.2 | 2020-12-17

### Changed

- Dark mode detection from JS 
- Tailwind CSS version 2
- The dark mode now is based on Tailwind colors
- Standard and Dark mode improvements

## 9.1.3 | 2020-07-19

### Changed

- HTML and JS cleanup
- Standard and Dark mode improvements

## 9.1 | 2020-07-18

### Changed

- Dark mode palette based on laneros blue color
- Fast loading and checking data
- Background adjustments on HTML

## 9.0.4 | 2020-07-17

### Changed

- Avatars adjustment

## 9.0.3 | 2020-07-16

### Changed

- Fixed alert user image

## 9.0.2 | 2020-07-16

### Changed

- Fixed alarm interval time

## 9.0.1 | 2020-07-15

### Added

- Emoji in alerts

## 9.0 | 2020-07-15

### Added

- Search
- Introduction of JS Modules

### Changed

- Code reorganization
- Dark theme update
- New icons
- Plugins version upgraded

## 8.3 | 2019-12-28

### Changed

- Localization adjustments
- Placeholder color
- Trophy points and feedback

## 8.2.1 | 2019-12-17

### Changed

- Popup counters adjustment

## 8.2 | 2019-12-17

### Changed

- Adjust the code to be valid HTML5
- Popup counters adjustment
- Minor javascript adjustments

## 8.1.5 | 2019-09-23

### Changed

- Adjust the code to be valid HTML5

## 8.1.3 | 2019-09-11

### Changed

- Tailwind version upgraded
- Locale messages fixed
- Minor HTML and javascript improvements

## 8.1.2 | 2019-07-11

### Changed

- Tailwind version upgraded
- jQuery validate version upgraded

## 8.1.1 | 2019-06-19

### Added

- Feedback filters

### Changed

- HTML adjustments

## 8.1 | 2019-06-19

### Added

- Feedback is back

### Removed

- Removed some links in Home tab 

## 8.0.3 | 2019-06-17

### Changed

- Locale updated
- CSS dark mode review
- javascript adjustments
- HTML improvements

## 8.0 | 2019-06-02

### Changed

- Locale updated
- jQuery upgrade
- CSS dark mode review
- Tailwind upgrade
- HTML improvements

## 7.6.2 | 2019-04-06

### Changed

- Dark mode CSS update

## 7.6.1 | 2019-04-06

### Changed

- CSS update
- HTML improvement

### Removed

- Unused images

## 7.6 | 2019-04-05

### Changed

- CSS update
- Upgrade tailwind
- Javascript update
- HTML improvement

## 7.5 | 2019-03-01

### Changed

- Notifications fix

## 7.4.5 | 2019-02-28

### Changed

- Version adjustment

## 7.4.4 | 2019-02-28

### Changed

- Version adjustment

## 7.4.3 | 2019-02-28

### Changed

- CSS adjustments
- Javascript adjustments 

## 7.4.2 | 2019-02-28

### Changed

- Dark mode header

## 7.4 | 2019-02-27

### Added

- Bookmarks

## 7.3.2 | 2019-02-14

### Changed

- CSS adjustments

## 7.3.1 | 2019-02-13

### Changed

- Javascript adjustments

## 7.3 | 2019-02-09

### Added

- Dark mode

### Changed

- HTML adjustments

## 7.2.3 | 2019-02-08

### Changed

- Status poster
- HTML updated

## 7.2.2 | 2019-02-08

### Changed

- HTML updated

## 7.2.1 | 2019-02-08

### Changed

- Developer mode fix

## 7.2 | 2019-02-08

### Added

- Token on requests

## 7.0.6 | 2018-12-26

### Changed

- Minor Javascript adjustments

## 7.0.5 | 2018-12-26

### Changed

- Remove some unused javascript

## 7.0.2 | 2018-12-24

### Changed

- Conversations CSS adjustment

## 7.0.1 | 2018-12-24

### Changed

- Minor HTML and text adjustments

## 7.0 | 2018-12-24

### Added

- Tailwind

### Changed

- Almost complete rewrite of the javascript
- Minor bug fixes

### Removed

- Bootstrap

## 6.6.5 | 2018-05-30

### Changed

- Conversations fix

## 6.6.4 | 2018-05-19

### Changed

- Conversations fix

## 6.6.3 | 2018-05-09

### Changed

- CSS improvement

## 6.6.2 | 2018-04-11

### Changed

- Avatar URL fix

## 6.6.1 | 2018-04-10

### Changed

- Subscriptions tab fix

## 6.6 | 2018-04-07

### Changed

- Bootstrap 4.0.0
- jQuery 3.3.1
- Javascript improvements
- CSS unification
- HTML modifications

## 6.5 | 2017-08-11

### Changed

- Bootstrap 4.0.0-beta
- CSS improvements
- Javascript improvements
- HTML updated

## 6.1.5 | 2017-05-21

### Changed

- Extension name

## 6.1.3 | 2017-05-21

### Changed

- Bootstrap 4.0.0-alpha.6
- CSS improvements
- Javascript improvements
- HTML modification to met bootstrap requirements
- jQuery updated to last version
- jQuery validate updated to last version

## 5.6.1 | 2016-11-10

### Changed

- CSS improvements
- Some URLs updated

## 5.6 | 2016-11-05

### Added

- Some buttons to notifications

### Changed

- Upgraded Bootstrap
- Updated jQuery to last version
- Updated jQuery From validate to last version

## 5.5.2 | 2016-03-27

### Changed

- User avatar fix

## 5.5.1 | 2016-03-20

### Changed

- User avatar fix

## 5.5 | 2016-03-20

### Changed

- Language improvements
- CSS improvements
- Major javascript changes

## 5.0.3 | 2016-03-07

### Changed

- Minor CSS adjustments
- Split user rating

## 5.0.2 | 2016-02-22

### Changed

- Minor javascript adjustments

## 5.0.1 | 2015-12-25

### Added

- Logging

### Changed

- Javascript improvements

## 5.0 | 2014-03-23

### Added

- Published on chrome web store

### Changed

- CSS improvements
- Javascript improvements
- Boostrap update to latest version
- jQuery update to latest version

### Removed

- Bootstrap Switch
- Background HTML page

## 4.0.3 | 2014-03-16

### Added

- Event Pages
- Alarms
- Storage
- Login
- Content reload

### Changed

- CSS improvements
- JS improvements

## 4.0 | 2014-03-09

### Added

- Bootstrap
- jQuery forms

### Changed

- Updated jQuery
- Major Design Update

## 3.3.3 | 2014-03-07

### Changed

- Design Update
- Update Messages

## 3.3.1 | 2014-03-07

### Changed

- Logo with transparent background

## 3.3 | 2014-03-07

### Changed

- Image logos
- Description
- CSS improvements

## 3.2.1 | 2014-02-18

### Changed

- Remove Laneros email link

## 3.2 | 2014-02-18

### Changed

- Forum URL fix

## 3.1 | 2014-02-18

### Changed

- Major Update
- Update jQuery
- Improve folder organization

## 3.0.3 | 2013-07-21

### Changed

- Quick URL fix

## 3.0.2 | 2013-07-21

### Changed

- CSS improvements
- JS improvements

## 3.0.1 | 2013-07-14

### Changed

- Fix inbox messages on popup

## 3.0 | 2013-07-12

### Added

- CSS popup
- Inbox
- Some shortcuts

### Changed

- Manifest v2
- Folder organization
- Split HTML and JS
- Notifications

## 2.6.8 | 2012-01-28

### Changed

- Subscriptions fix

## 2.6.6 | 2011-12-04

### Changed

- Shortcuts fix

## 2.6.5 | 2011-12-03

### Added

- Shortcuts

## 2.6.1 | 2011-12-02

### Changed

- Subscriptions bug fix

## Removed

- Unofficial title

## 2.6 | 2011-11-29

### Added

- Login form

## 2.5.2 | 2011-11-28

### Changed

- Badge update fix

## 2.5.1 | 2011-11-28

### Added

- Localization (EN, ES)

### Changed

- CSS, JS, HTML with minor changes

## 2.0.5 | 2011-11-26

### Added

- Running check

## 2.0.4 | 2011-11-26

### Added

- Global counter

## 2.0.3 | 2011-11-26

### Changed

- Minor bugs fixed

## 2.0.2 | 2011-11-26

### Changed

- Fixed "true" value

## 2.0.1 | 2011-11-26

### Changed

- Notifications function

## 2.0 | 2011-11-26

### Added

- Some extension options
- jQuery

### Changed

- Notification icons
- Notification Popup
- Options page
- Javascript functions (notifications, subscriptions, popup, options)

## 1.3.5 | 2011-11-25

### Added

- Number of notifications on extension icon
- Notification icons
- Setting page

## 1.3.2 | 2011-11-25

### Added

- Laneros Logo 

### Changed

- CSS: colors, classes, fonts, etc
- HTML: UTF-8

### Removed

- Text without notifications

## 1.3.1 | 2011-10-30

### Added

- Auto updates
- Subscriptions

## 1.3 | 2011-10-27

### Added

- Desktop notifications

## 1.2 | 2011-10-27

### Added

- Notifications on Popup

## 1.1 | 2011-10-26

### Added

- HTML page

## 1.0 | 2011-10-26

- Initial release
